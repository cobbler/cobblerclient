...
// FindMgmtClass is searching for one or more managementclasses by any of its attributes.
...
// CopyMgmtClass is copying a given managementclass server side with a new name.
func (c *Client) CopyMgmtClass(objectId, newName string) error {
	_, err := c.Call("copy_mgmtclass", objectId, newName, c.Token)
	return err
}

// RenameMgmtClass is renaming a managementclass with a given object id.
func (c *Client) RenameMgmtClass(objectId, newName string) error {
	_, err := c.Call("rename_mgmtclass", objectId, newName, c.Token)
	return err
}

// GetMgmtClassesSince is returning all managementclasses which were created after the specified date.
...
// GetMgmtClassAsRendered is returning the datastructure after it has passed through Cobblers inheritance structure.
...
// SaveMgmtClass is persisting all changes performed via XML-RPC to disk on the server side.
